...
export class Potree implements IPotree {
  ...
  updatePointClouds(
    ...
    for (let i = 0; i < pointClouds.length; i++) {
      ...
      if (
        attIntensity != null &&
        pointCloud.material.intensityRange[0] === Infinity
      ) {
        pointCloud.material.intensityRange = [
          ...(Object.hasOwn(attIntensity, "range")
            ? ((attIntensity as PointAttribute).range as [number, number])
